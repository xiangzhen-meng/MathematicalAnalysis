...
>[!question] 经典不等式的证明
>求证：$$\frac{x}{1+x}<\ln(1+x)<x$$

可以使用带 Lagrange 余项的 Taylor 展开进行证明

令 $f(x)=\ln(1+x)$，则将其在 $x=0$ 处展开到二阶，得到：
$$
f(x)=\ln(1+x)=x+\frac{f''(\xi)}{2!}x^2=x- \frac{1}{2(\xi+1)^2}x^2
$$
其中，$\xi\in(0,x)$ ，对右半侧显然有：
$$
f(x)=x- \frac{1}{2(\xi+1)^2}x^2<x
$$
另一方面，将其在 $x=0$ 处展开至一阶，得到：
$$
f(x)=0+\frac{f'(\xi)}{1}x=\frac{x}{1+\xi}
$$
其中，$\xi\in(0,x)$，则对 $\xi$ 进行估计，得到：
$$
