...
- [x] 这里补充达布和的收敛 📅 2026-01-07 ✅ 2026-01-08
...
如果函数是 [[闭区间连续函数]]，则必定黎曼可积，这是由 [[一致连续性]] 保证的

进一步推广，如果仅有有限个间断点，则函数也是可积的

### 充分条件 2 单调性

闭区间上的单调函数必定黎曼可积

进一步推广，闭区间上的 **有界变差函数** 也必定黎曼可积
...
>[!note] 例 1 指数函数复合后保可积性
>$f(x)\in R[a,b]$ 求证：$\exp(f(x))\in R[a,b]$

事实上，我们想要证明 $\sum\omega_k[\exp(f(x))]\Delta x_k$ 是趋于零的。而由已知，有 $\sum\omega_k(f(x))\Delta x_k$ 趋于零。这两个之间差了一个指数

使用中值定理
$$
\begin{align}
\omega(\exp(f(x)))&=|\exp(f(\xi))||f(x_{1})-f(x_{2})| \\
&\leq e^M|f(x_{1})-f(x_{2})|
\end{align}
$$
于是有：
$$
\sum\omega_k(\exp(f(x)))\Delta x_k\leq \sum e^M\omega_k(f(x))\Delta x_k
$$
有限数乘无穷小仍然是无穷小，证毕